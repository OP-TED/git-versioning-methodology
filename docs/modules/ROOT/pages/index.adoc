...
* Folder structure of the project is consistent and coherent in the evolution history and across the branches. 
...
* Each major, minor and patch versions are pushlished separate tags. 
* Branches shall not be used for versioning but for specific functional purposes, suchs as, individual bug fixing, individual new feature developments, etc. 
* Allow to transit smoothly from one major version to the next one without disrupting the current clients. Clients shall be guaranteed to have no disruptions for a published version for an established time frame.
* Clients that are unable to migrate to new versions might be be supported.
